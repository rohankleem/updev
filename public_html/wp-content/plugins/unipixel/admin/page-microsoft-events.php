...
/**
 * Microsoft → Events page (PageView / WooCommerce / Custom)
 * Flat layout, single form + single "Update All" button.
 */
function unipixel_page_microsoft_events()
{
    global $wpdb;
    ...
    $pixelNameFriendly  = 'Microsoft UET';
    ...
    // Extract / defaults
    if ($platformSettings) {
        $pageview_send_serverside          = isset($platformSettings['pageview_send_serverside']) ? (int) $platformSettings['pageview_send_serverside'] : 0;
        $pageview_send_clientside          = isset($platformSettings['pageview_send_clientside']) ? (int) $platformSettings['pageview_send_clientside'] : 1;
        $pageview_send_server_log_response = isset($platformSettings['send_server_log_response']) ? (int) $platformSettings['send_server_log_response'] : 0;
    } else {
...
?>
    <div class="UniPixelShell position-relative" data-platform="microsoft">
        ...
        <?php unipixel_render_platform_header_nav('microsoft', 'events'); ?>
        ...
        <form id="unipixel-events-all-form" class="position-relative">
            ...
            <!-- Section 1: PageView   -->
            <!-- ===================== -->
            <section id="microsoft-pageview-section" class="mb-5">
                <h2 class="mb-2"><?php echo esc_html__('PageView Event', 'unipixel'); ?></h2>
                <p class="text-muted mb-3">Microsoft UET automatically handles PageView tracking when the tag is installed.</p>
                ...
                <table class="table table-striped table-bordered align-middle mb-0">
                    <thead>
                        <tr>
                            ...
                            <th class="colSendClientSide">Send<br/>Client-side</th>
                            <th class="colSendServerSide">Send<br/>Server-side</th>
                            <th class="colLogResponse">Log Server-side Response</th>
                    ...
                    <tbody>
                        <tr>
                            <td>PageView</td>
                            <td>Automatically tracked by Microsoft UET.</td>
                            <td>
            ...
            <!-- ===================================== -->
            <section id="microsoft-woo-section" class="mb-5">
            ...
            <!-- ================================= -->
            <section id="microsoft-custom-section" class="mb-3">
                <h2 class="mb-2"><?php echo esc_html__('Site Events for', 'unipixel'); ?> <?php echo esc_html($platformName); ?></h2>
                <p class="mb-3">
                    <?php echo esc_html__('Track form submissions, clicks, visible elements, or page URL matches. For each event pick a Trigger, what it acts on, then a Platform Event Reference (sent as the UET Event Action): a Standard name recognised by', 'unipixel'); ?> <?php echo esc_html($platformName); ?><?php echo esc_html__(', or a Bespoke one of your own.', 'unipixel'); ?>
                ...
                <table id="event-settings-table" class="table">
                    <thead>
                        <tr>
                            ...
                            <th><?php echo esc_html__('Platform Event Ref', 'unipixel'); ?><br><small class="fw-normal"><?php echo esc_html__('sent to', 'unipixel'); ?> <?php echo esc_html($platformName); ?>. <?php echo esc_html__('e.g.', 'unipixel'); ?> <code>purchase</code>, <code>MyBespokeEvent</code></small></th>
