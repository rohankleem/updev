...
// Column Titles (replace with actual column titles in your board)
define('TIMELINE_COLUMN_TITLE', 'Timeline'); // Title of the Timeline column in sub-items
...
// Fetch column IDs for a board or sub-item
function fetch_column_ids($boardId, $isSubItem = false) {
    write_to_log("[INFO] Fetching column IDs for " . ($isSubItem ? "sub-item" : "board") . " ID: $boardId");
    $query = json_encode([
        'query' => "query {
            boards(ids: {$boardId}) {
                columns {
    ...
    if (empty($response['data']['boards'][0]['columns'])) {
        write_to_log("[ERROR] No columns found for " . ($isSubItem ? "sub-item" : "board") . " ID: $boardId");
        return null;
    }
    ...
    write_to_log("Column Map for " . ($isSubItem ? "sub-item" : "board") . " ID $boardId: " . print_r($columnMap, true));
    return $columnMap;
}
...
function handle_webhook($payload) {
    ...
    if (isset($payload['event']['type']) && $payload['event']['type'] === 'update_column_value' && $payload['event']['columnId'] === 'date') {
        ...
        if (get_sub_item_count($item_id) === 0) {
            // Add sub-items
            $subItemColumnMap = fetch_column_ids($board_id, true); // Fetch column IDs for sub-items
            $timelineColumnId = $subItemColumnMap[TIMELINE_COLUMN_TITLE] ?? null;
