...
- **Token-acquisition UX initiative — all 8 phases shipped (#34 + subsumes #8 + #30 + #1).** Full plan and per-phase detail: `projects/token-acquisition-ux.md`. Headline pieces:
  ...
  - **14-day Log Server-side Response grace period** — when a token is first added per platform, response logging auto-enables for all events for that platform for 14 days, then a scheduled WP-Cron resets it to off. Surfaces a banner on the setup page while the grace is active and a one-time intro on the Stored Event Logs page.
