...
- **Live on wordpress.org:** `v2.6.9` (SVN-committed by Rohan; source committed + pushed `3e4c838`).
- **Releasing now:** `v2.6.10` — single feature: manual "Delete Stored Event Logs" on the General Settings page (with a text pointer to it from the Event Logs page). Gate files bumped, obfuscated export built, staged into SVN `tags/2.6.10/` + `trunk/readme.txt`. **Awaiting Rohan's TortoiseSVN commit.**
- **Still to do for 2.6.10:** (1) Rohan TortoiseSVN commit (`tags/2.6.10/` + `trunk/`); (2) bump `UPHQ_PLUGIN_VERSION` → 2.6.10 on uphq + deploy (gate file #5); (3) commit the `main` source (feature + bump) to git.
