...
- **Live on wordpress.org:** `v2.6.10` (SVN-committed and deployed by Rohan; source commit `206cfc6`).
- **Releasing now:** `v2.6.11` — two fixes from the steelchief.com.au investigation (2026-07-12): front-end de-jQuery (#26 / WP-001) and the Meta Test Connection rebuild (#30 / META-003). Obfuscated 2026-07-13, all pre/post-export checks passed, SVN working copy staged (`tags/2.6.11/` new + `trunk/readme.txt` updated to Stable tag 2.6.11). **Remaining:** source git commit + push, Rohan's TortoiseSVN commit, gate file #5 (uphq `UPHQ_PLUGIN_VERSION` bump + deploy).
- **Still to do for 2.6.11:** (1) commit + push `main`; (2) obfuscation export with pre-export checks; (3) Rohan TortoiseSVN commit (`tags/2.6.11/` + `trunk/`); (4) bump `UPHQ_PLUGIN_VERSION` → 2.6.11 on uphq + deploy (gate file #5).
...
| 26 | Remove jQuery dependency from frontend JS | Housekeeping | Days | **Done (staged 2.6.11)** | Front-end JS is now jQuery-free (`unipixelOnReady` + `unipixelAjaxPost` helpers in `unipixel-common.js`; `jquery` removed from the dependency array). Was the root cause of the steelchief.com.au Lead outage: the theme deregisters `jquery` on non-Woo pages and WP silently drops the whole watcher chain — Meta Lead events at zero for ~a month (2026-06 → 2026-07-12). Full incident: `domain-knowledge/platform-discoveries.md` § WP-001. Admin JS and Woo add-to-cart inline wrappers still use jQuery by design (guaranteed present there). |
...
| 30 | Meta test event code field | Event Quality, UX | Hours | **Done (staged 2.6.11)** | Meta Test Connection now auto-fires a `test_event_code`-tagged event (`TEST_UNIPIXEL`) to the Pixel and the success message points the user at the Test events tab. This replaced the scope/node-read checks that wrongly rejected dataset-scoped tokens (see `platform-discoveries.md` § META-003). A user-facing input for a custom code remains open if anyone asks. |
...
- **v2.6.11** (staged 2026-07-12, pending obfuscation export + Rohan's SVN commit) — Two fixes from the steelchief.com.au investigation. (1) **Front-end de-jQuery** (#26, root cause of WP-001): `unipixel-common` no longer declares the `jquery` handle as a dependency, and all front-end JS is jQuery-free. New shared helpers in `js/unipixel-common.js`: `unipixelOnReady()` (readyState-aware, fires under defer/delay optimisers where a bare DOMContentLoaded listener would miss) and `unipixelAjaxPost()` (fetch + URLSearchParams with jQuery.param-compatible bracket encoding for nested payloads, resolves parsed JSON). All five `clientfirst-watch-and-send-*.js` converted; Google/TikTok lose their `(function($){...})(jQuery)` wrappers. Admin JS and Woo add-to-cart inline wrappers intentionally keep jQuery (wp-admin and Woo both guarantee it). (2) **Meta Test Connection rebuilt** (META-003): the `ads_management` scope gate and the Pixel-node read both rejected the dataset-scoped tokens Events Manager generates today; replaced with an authoritative test-event send (`test_event_code TEST_UNIPIXEL` to `/{pixel_id}/events`, confirms `events_received`), plus a wrong-Pixel diagnostic from `granular_scopes.target_ids`. Also delivers the #30 Meta test-event path. Verified on updev with WP jQuery deregistered (steelchief simulation): full chain enqueues, zero console errors, organic PageView log POST 200, wire format byte-identical for flat and nested payloads; Graph calls proven live against production Meta from the steelchief server (both token shapes).
- **v2.6.10** (staged 2026-06-15, since shipped to wordpress.org) — Single feature: manual **Delete Stored Event Logs** from the General Settings page (blue `btn-primary` button in the "Store Records of Events to Database" section + a friendly Bootstrap confirm modal) that wipes `wp_unipixel_event_log` and resets the `wp_unipixel_log_count` gauge to 0 (nonce + `manage_options` gated). The Event Logs page has **no** button (it crowded the header); its intro text points to General Settings instead. New files: `handlers/handler-clear-event-logs.php`, `inc/clear-event-logs.php`; edits: `admin.php` (includes), `page-general-settings.php`, `page-event-logs.php`, `admin/js/admin-common.js`. Per-version user-facing detail: `readme.txt` changelog.
- **v2.6.9** (2026-06-15, shipped) — Token-acquisition UX initiative shipped (per-platform setup wizards, Test Connection buttons with platform-specific diagnostics, three-state status strips, home dashboard badges, plain-English Stored Event Logs response labels, client-side/server-side setup-page split, credential-field autofill hardening). Plus the `wp_unipixel_event_log.log_time` index (originally the 2.6.8 hotfix) and three fixes from the 2026-06-15 release-readiness test pass: (1) **log prune was completely inert** — `cleanup_logs()` passed the ID array as a single `wpdb::prepare()` arg, so the DELETE collapsed to an empty query and the event log grew unbounded; fixed with arg spread, count gauge now mirrors the true row count, thresholds retuned to 20k trigger / 10k delete. (2) **platform-settings save JSON error** — `handler-platform-settings.php` read `$_POST['pageview_send_serverside']` unguarded (the JS never sends it), emitting an Undefined-array-key warning that corrupted the AJAX JSON on hosts with `display_errors` on (the reported "Ajax request failed" / JSON.parse bug). (3) **silent PageView clobber** — same missing field meant every setup-page save rewrote `pageview_send_serverside` to 0, undoing the events-page toggle; now only written when posted. Also softened Google's connection badge to "Format checks passed" (MP validation can't confirm the API secret). Note from testing: the `log_time` index is largely cosmetic for the prune itself (MySQL ignores it at `LIMIT 10000`); the working prune is the real fix. Per-version user-facing detail: `readme.txt` changelog.
